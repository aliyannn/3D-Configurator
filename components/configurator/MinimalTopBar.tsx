...
const CATEGORIES = [
  { id: "car_gtx", label: "Vehicles", icon: "🚗" },
  { id: "bike_valkyrie", label: "Bikes", icon: "🏍️" },
  { id: "sofa_haven", label: "Furniture", icon: "🛋️" },
  { id: "sneaker_apex", label: "Sneakers", icon: "👟" },
  { id: "headphones_aura", label: "Tech", icon: "🎧" },
];
...
export function MinimalTopBar({ onCaptureSnapshot }: MinimalTopBarProps) {
  ...
  return (
    <header className="pointer-events-auto w-full flex items-center justify-between gap-3 max-w-7xl mx-auto px-4 pt-4 select-none">
      {/* 1. Left: Product Title & Active Category Pill */}
      <div className="flex items-center gap-2.5 px-4 py-2 rounded-full bg-zinc-950/70 border border-white/10 backdrop-blur-2xl shadow-xl">
        ...
        <div className="flex items-center gap-1.5 text-xs">
          ...
          <span className="text-zinc-300 font-medium truncate max-w-[140px] sm:max-w-xs">
            {currentModel.title}
          </span>
        </div>
      </div>

      {/* 2. Center: Clean Floating Category Switcher */}
      <nav className="hidden md:flex items-center gap-1 p-1 rounded-full bg-zinc-950/70 border border-white/10 backdrop-blur-2xl shadow-xl">
        ...
        <button
          ...
          title="Upload or link custom .GLB 3D model"
      ...
      {/* 3. Right: Snapshot & Save Spec Sheet CTAs */}
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-full text-xs font-medium text-zinc-300 bg-zinc-950/70 hover:bg-zinc-900 border border-white/10 hover:border-white/20 backdrop-blur-2xl shadow-xl active:scale-95 transition-all"
        >
